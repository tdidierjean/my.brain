...
(function(jQuery) {
	...
	jQuery.simpleautogrow = function (e) {
		...
		var $e = this.textarea = jQuery(e)
			...
			.bind('focus', function() {
				this.timer = window.setInterval(function() {self.checkExpand(); }, 400); }) //upped from 200
	...
	jQuery.simpleautogrow.prototype.checkExpand = function() {
		...
		if (this.textarea.outerHeight() != target_height && target_height > 15) //added min target_height check
			this.textarea.height(target_height + 15 + 'px'); //added extra empty line
